...
public class InRegion extends Controller {
    ...
    private JLabel creditDisplay;
    private JButton repairShip;
    private JButton winGame;
    private Random rand = new Random();
    private JDialog buyingItem;
    private int price = 1000 * Game.difficulty.getDifficulty();
    ...
    private void screenSetup() {
        ...
        repairShip = new JButton("Repair ship");
        repairShip.setAlignmentX(Component.CENTER_ALIGNMENT);
        repairShip.addActionListener((ActionEvent e) -> {
            double cost = ShipController.determineRepairCost();
            int wantsToRepair = JOptionPane.showConfirmDialog(
                    mainFrame, "This will cost "
                    + df.format(cost)
                            + " credits. Are you sure you want to repair your "
                    + "ship?", "Ship Repair", JOptionPane.YES_NO_OPTION);
            boolean success = Game.playerOne.getShip().repairShip(
                    wantsToRepair, healthInfo, cost, creditDisplay);
            if (!Game.playerOne.getShip().isShipDamaged()) {
                repairShip.setVisible(false);
            } else if (!success) {
                JOptionPane.showMessageDialog(mainFrame,
                        "You didn't have enou"
                        + "gh money for the repair.", "Could not repair",
                        JOptionPane.ERROR_MESSAGE);
            }
        });

        winGame = new JButton(Game.playerOne.getName()
                + "'s Universe!");
        winGame.setAlignmentX(Component.CENTER_ALIGNMENT);
        winGame.addActionListener((ActionEvent e) -> {
            buyItem();
        });

        shipInfo = new JLabel();
        shipInfo.setText("Current ship: "
                + ShipController.getShipType());
        ...
        if (Game.playerOne.getShip().isShipDamaged()) {
            newGame.add(repairShip);

        }
        newGame.add(shipInfo);
        newGame.add(travelButton);
        if (Game.playerOne.getCurrentLocation().getName().equals(
                getRandomRegion().getName())) {
            newGame.add(winGame);
    ...
    /**
     * Method for buying the "universe", aka the winning item
     */
    private void buyItem() {
        buyingItem = new JDialog();
        buyingItem.setVisible(true);
        buyingItem.setMinimumSize(new Dimension(400, 275));
        buyingItem.setMaximumSize(new Dimension(400, 275));
        buyingItem.setLocationRelativeTo(mainFrame);
        buyingItem.getRootPane().setLayout(new BoxLayout(
                buyingItem.getRootPane(), BoxLayout.Y_AXIS));


        JLabel explain = new JLabel("Do you want to buy this item?"
                + " It is " + price + " credits.");
        JButton buy = new JButton("Buy");
        JButton cancel = new JButton("Cancel");

        buyingItem.getRootPane().add(explain);
        buyingItem.getRootPane().add(buy);
        buyingItem.getRootPane().add(cancel);



        buy.addActionListener((ActionEvent e) -> {
            if (Game.playerOne.getCredits() >= price) {
                buyingItem.dispose();
                winDisplay();
            } else {
                buyingItem.dispose();
                JOptionPane.showMessageDialog(mainFrame, "Sorry, you "
    ...
    private void winDisplay() {
        ...
        JLabel winning = new JLabel("<html>"
                + "Congratulations! You Win! "
                + "</html>");
        winning.setAlignmentX(Component.CENTER_ALIGNMENT);

        JPanel buttonPane = new JPanel();
        buttonPane.setLayout(new BoxLayout(buttonPane, BoxLayout.X_AXIS));
        JButton startOver = new JButton("Start Over");
        startOver.addActionListener((ActionEvent) -> {
            winFrame.dispose();
        ...
        quit.addActionListener((ActionEvent) -> {
            winFrame.dispatchEvent(
                    new WindowEvent(winFrame, WindowEvent.WINDOW_CLOSING));
        });
        ...
        winFrame.add(panel);
        winFrame.setPreferredSize(new Dimension(800, 800));
    ...
    private void marketplaceSetup() {
        ...
        creditDisplay = new JLabel("Current credits: "
                + df.format(Game.playerOne.getCredits()));
        cargoDisplay = new JLabel("Cargo: "
    ...
    private void regionSetup() {
        ...
        goTo.addActionListener((ActionEvent event) -> {
            if (canTravel()) {
                ...
                JDialog currentDialog = EncounterGenerator.determineEncounter(
                        Game.difficulty, Game.playerOne);
                if (currentDialog != null) {
                    currentDialog.setUndecorated(true);
                    currentDialog.getRootPane().add(NPCInteraction.iconHolder);
                    mainFrame.setTitle("An Encounter Occurred!");
                    JPanel blank = new JPanel();
                    mainFrame.setContentPane(blank);
                    mainFrame.setVisible(true);
                    currentDialog.addWindowListener(new WindowListener() {
                        @Override
                        ...
                        public void windowClosed(WindowEvent e) {
                            NPCInteraction.iconHolder = new JPanel();
                            if (!NPCInteraction.getGoBack()) {
                                info.setVisible(false);
                                returnPanel.setVisible(false);
                                newGame.setVisible(false);
                                new InRegion(
                                        curr, game, currentList);
                                // mainFrame.dispose();
                            } else {
                                info.setVisible(false);
                                returnPanel.setVisible(false);
                                newGame.setVisible(false);
                                new InRegion(initialRegion, game, currentList);
                                // mainFrame.dispose();
                            }
                        }
                        @Override
                    ...
                    mainFrame.setContentPane(iconHolder);
                } else {
                    info.setVisible(false);
                    returnPanel.setVisible(false);
                    newGame.setVisible(false);
    ...
    private void mapSetup() {
        ...
        JButton button1 = new RoundButton(currentList.get(1).getName(),
        ...
        JButton button2 = new RoundButton(currentList.get(2).getName(),
        ...
        JButton button3 = new RoundButton(currentList.get(3).getName(),
        ...
        JButton button4 = new RoundButton(currentList.get(4).getName(),
        ...
        JButton button5 = new RoundButton(currentList.get(5).getName(),
        ...
        JButton button6 = new RoundButton(currentList.get(6).getName(),
        ...
        JButton button7 = new RoundButton(currentList.get(7).getName(),
        ...
        JButton button8 = new RoundButton(currentList.get(8).getName(),
        ...
        JButton button9 = new RoundButton(currentList.get(9).getName(),
    ...
    private void displayRegion(Region curr) {
        ...
        fuelCost = distance / (Game.playerOne.getPilotPoints() + 1);
    ...
    /**
     * finds a random region
     * @return random region
     */
    private Region getRandomRegion() {
        if (randomRegion == null) {
            int var = rand.nextInt(10);
            randomRegion = regions.get(var);
            //System.out.println(randomRegion.getName());
            return regions.get(var);
        } else {
            return randomRegion;
        }
    }
